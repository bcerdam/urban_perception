...
def validate_model(weight_path, pp2_validation, m_w, m_t, similarity_threshold):
    ...
    with torch.no_grad():
        for vote in pp2_validation:
            ...
            loss = utils.loss(left_score, right_score, label, m_w, m_t, device)
    ...
    accuracy = ((count_left+count_right+count_tie)/len(pp2_validation))

    output = (f'Validation Loss: {running_loss/len(pp2_validation)}, Validation Accuracy: {accuracy}\n'
